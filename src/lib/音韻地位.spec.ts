...
test('正則化', t => {
  ...
  t.is(音韻地位.from描述('並咍上').正則地位('')[0].描述, '並一灰上');

  t.false(音韻地位.from描述('定開脂去').is正則地位('草'));
  t.true(音韻地位.from描述('定開脂去').is正則地位('地'));
  t.true(音韻地位.from描述('定開脂去').is正則地位(''));

  t.is(音韻地位.from描述('云開仙平').is正則地位(''), 'weak');
  t.not(音韻地位.from描述('云開仙平').正則地位('')[1], null);
});
