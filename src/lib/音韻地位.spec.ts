...
test('最簡描述 & from描述', t => {
  ...
  // 赓清最簡描述省略重紐，`from描述` 時填入
  t.is(from('云合三庚上').描述, '云合三B庚上');
  t.is(from('云合三庚上').最簡描述, '云合三庚上');
  t.is(from('明清平').描述, '明三A清平');
  t.is(from('明清平').最簡描述, '明清平');
  // 蒸幽重紐非必須，不作此處理
  t.is(from('云合蒸入').描述, '云合三蒸入');
  t.is(from('云合B蒸入').描述, '云合三B蒸入');
});
