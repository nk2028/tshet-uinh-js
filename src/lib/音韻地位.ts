...
export class 音韻地位 {
   ...
   * 嵌入的參數可以是：
   *
   * * 函數：會被執行；若其傳回值為字串，會視作表達式，遞迴套用{@linkcode 屬於}來判斷，否則會直接檢測其真值
   * * 字串：視作表達式，遞迴套用{@linkcode 屬於}
   * * 其他：會檢測其真值
   *
   * **注意**：
   *
   * * 該語法僅能用於字面值模板串，不能用於如{@linkcode 判斷}等
   ...
   * 判斷式可以是：
   *
   * * &#x3000;&#x3000;函數：會被執行；若其傳回值為非空字串，會套用至{@linkcode 屬於}函數判斷是否符合，若為布林值則由該值決定是否符合本條件，若為其他值則直接視作符合本條件
   * * 非空字串：描述音韻地位的表達式，會套用至{@linkcode 屬於}函數
   * * &#x3000;布林值：直接決定是否符合本條件
